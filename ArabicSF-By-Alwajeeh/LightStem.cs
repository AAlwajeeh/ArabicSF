...
namespace ArabicSF
{
    public class LightStem
    {
        ...
        private List<object> listsVector = new List<object>();
        ...
        public LightStem(string fileToBeStemmed, List<string> statFiles)
        {
            setInit(fileToBeStemmed, statFiles);
        }
        private void setInit(string fileToBeStemmed, List<string> statFiles)
        {
            try
            {
                string str = LightStem1(fileToBeStemmed, statFiles);
                File.WriteAllText(fileToBeStemmed, str);
            }
            catch (IOException)
            {
                setInit(fileToBeStemmed, statFiles);
        ...
        public String LightStem1(string fileToBeStemmed, List<string> statFiles)
        {
            ...
            text.Append("");
            string [] stopW = staticFiles[14].Split(' ');
            //Keep these Stop words
            string[] stopWExcept = {""};
            //bool flag = false;
            for (int i = 0; i < listStemmedWords.Count; i++)
            {

                /* if (stopWExcept.Contains(listStemmedWords[i]))
                 {
                     text.Append(listStemmedWords[i]);
                     text.Append(" ");
                     continue;
                 }
                 else if(stopW.Contains(listStemmedWords[i]))
                     continue;*/
                for (int x = 0; x < definiteArticles.Length; x++)
                {
                    if(listStemmedWords[i].StartsWith(definiteArticles[x]))
                    {
                        listStemmedWords[i] = listStemmedWords[i].Substring(definiteArticles[x].Length);
                        //flag = true;
                        break;
                    }
                }
                ...
                {
                    if (listStemmedWords[i].StartsWith(prefixes[j]) && !listRootsFound[i].StartsWith(prefixes[j]))
                    {
                        listStemmedWords[i] = listStemmedWords[i].Substring(prefixes[j].Length);
                        break;
                    }
                }

                for (int y = 0; y < suffixes.Length; y++)
                {
                    if (listStemmedWords[i].EndsWith(suffixes[y]) && !listRootsFound[i].EndsWith(suffixes[y]))
                    {
                        string temp = listStemmedWords[i];
                        listStemmedWords[i] = listStemmedWords[i].Substring(0, listStemmedWords[i].Length - suffixes[y].Length);
                        if(suffixes[y].CompareTo("ات") == 0)
                            listStemmedWords[i] += "ة";
                        if (listStemmedWords[i].Length < 3)
                            listStemmedWords[i] = temp;
                        break;
                    }
        ...
        private void readFromFile(string currentFile)
        {
            ...
            String[] br = File.ReadAllLines(currentFile);//new InputStreamReader ( in, "UTF-16" );
